...
import postsPageInitialPathsF212eResource from '../../resources/posts-page-initial-paths-f212e'
import postsPageInitialProps43592Resource from '../../resources/posts-page-initial-props-43592'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Principal Operations Associate</title>
          <meta
            property="og:title"
            content="Posts1 - Principal Operations Associate"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPathsF212eResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialProps43592Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
